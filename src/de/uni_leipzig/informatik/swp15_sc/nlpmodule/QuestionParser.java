...
public class QuestionParser {
	 ...
	 * @param independentEntriesFile path to file containing the independent lexicon
	 */
	 ...
	 * @param words tagged words of the question
...
//		}
		...
		for (int i=0; i<words.length; i++) {
			for (int j=i; j<words.length; j++) {
				try {
					...
					// if j-loop ends (end of current subsentence is end of sentence) and no matching
					// entry was found, add the first word (i) of the subsentence to remainingwords
					if (j+1 == words.length && isRelevant(words[i])) factory.addWord(words[i]);
					continue;
				}
			}
		}
	...
	private boolean isRelevant (TaggedWord word) {
		// is it a name?
		if (!word.getNe().equals("O")) return true;
